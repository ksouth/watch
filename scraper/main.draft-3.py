...
def main():
    ...
    links="\n".join(f'<li><a href="{html.escape(Path(x["path"]).relative_to(root).as_posix(),quote=True)}">{html.escape(urlsplit(x["url"]).path or "/")}</a></li>' for x in sorted(active,key=lambda x:x["url"]))
